...
# InvenioRDM v14 recommends Python 3.14; the migration upgrades the instance to it
...
def normalize_version(version: str) -> str:
    """Convert a Pipfile version specifier to PEP 508 form (`*` means any version)."""
    return "" if version.strip() == "*" else version
...
def parse_pipfile_dependency(dep_spec: Union[str, dict]) -> Optional[str]:
    ...
    if isinstance(dep_spec, str):
        return normalize_version(dep_spec)
    ...
    if isinstance(dep_spec, dict):
        ...
        if "version" in dep_spec:
            parts.append(normalize_version(dep_spec["version"]))
...
def convert_pipfile_to_pyproject(config: ProjectConfig) -> str:
    """Convert Pipfile to pyproject.toml and return the target Python version."""
        ...
        if isinstance(spec, dict) and "path" in spec:
            ...
            if isinstance(spec, dict) and "extras" in spec:
                ...
                if isinstance(extras, list):
                    ...
                    extras_str = extras
                version = normalize_version(spec.get("version", ""))
...
def write_python_version(root_dir: Path, python_version: str) -> None:
    """Pin the target Python version by writing `.python-version`."""
    version_file = root_dir / ".python-version"
    version_file.write_text(f"{python_version}\n")
    click.secho(f"✓ Pinned Python {python_version} in {version_file}", fg="green")
...
def cleanup_old_files(root_dir: Path) -> None:
    ...
    # Stale *.egg-info directories can shadow the new entry points, so drop them
    for egg_info in site_dir.glob("*.egg-info"):
        shutil.rmtree(egg_info)
        removed_files.append(egg_info)
        click.secho(f"🗑️ Removed {egg_info}", fg="yellow")
...
def main(root_dir: Path, project_name: Optional[str], cleanup: bool):
    ...
    if project_name is None:
        ...
        try:
            ...
            # Extract author info from .invenio
        ...
        author_name = "CHANGE_ME"
        author_email = "change@me.org"
        click.secho("⚠️  Using default values for author", fg="yellow")
        click.secho(
    ...
    try:
        ...
        python_version = convert_pipfile_to_pyproject(project_config)
