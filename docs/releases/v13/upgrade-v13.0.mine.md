# Upgrading from v12.0 to next


Because of changes to Members, we need to init and rebuild those indices.
...
Make sure you have the latest `invenio-cli` installed. For InvenioRDM vXX it
is at least vX.Y.Z

```bash
$ invenio-cli --version
invenio-cli, version X.Y.Z
```
