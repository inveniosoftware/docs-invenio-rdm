...
The following are **baseline minimum requirements** for an InvenioRDM production deployment. On their own, they correspond to a small instance. Adjust the resources based on your expected traffic, data volume, and performance requirements.

**Lightweight services:**

- HAProxy (if used at all): 0.1 CPU, 100 MB memory
- Nginx: 0.1 CPU, 100 MB memory
- Redis: 0.1 CPU, 250 MB memory (use RabbitMQ recommendations below if also used as queue)
- RabbitMQ (if Redis not used for queue as well): 0.5 CPU, 1 GB memory
...
A single server with 4 vCPUs and 16 GB memory is typically sufficient if all services are co-located.
If multiple servers are used (via docker-compose's `network_mode: host` for example), they can each be scaled down versions i.e., 1-2 vCPUs and 2-8 GB.
...
- Mount volumes or use bind mounts for all services (database, search, cache, queue) to persist data across restarts
- For local file storage, **backup** your volumes or bind mount to prevent data loss
...
- Ensure remote connectivity from application servers
- **Restrict external network access**
- Implement **backup and recovery** strategies at least for statistics
  * Except for statistics, all important data in OpenSearch can be reindexed from the database
  * [Views and downloads statistics](../../maintenance/internals/statistics.md) are **only stored in OpenSearch** (not the database)
  * You can follow [the instructions here](./backup_search_indices.md) to back them and/or other indices
...
You can spy the Dockerfile of the official [InvenioRDM Docker images](../../maintenance/operations/docker-images.md) for what packages to install if not using those images.

Also look at the [Compose file of the latest cookiecutter template](https://github.com/inveniosoftware/cookiecutter-invenio-rdm/blob/master/%7B%7Bcookiecutter.project_shortname%7D%7D/docker-compose.full.yml) to see the most up-to-date commands that the different services should run. The ones transcribed below in this documentation page may become out-of-date, but the ones at that link will always be the most up-to-date.
...
Review the application server configuration (uWSGI at time of writing) and run the server.
...
**Security:** Only the load balancer/reverse proxy should be internet-facing. All other infrastructure components must be isolated from external network access.
...
2. **Create an admin user**
    ...
    ```bash
    $ invenio users create admin@your-institution.org --password ... --active --confirm
    ```

    - Replace `admin@your-institution.org` with your admin email
    - The `--active` and `--confirm` flags activate and confirm the account immediately
...
Many services exist that can catch and provide detailed exception information (variables, stack traces, etc.). [Sentry](https://sentry.io/welcome/) is a popular one. Registering a Python logger to send emails with stacktraces is also a rugged possibility.
...
- Get detailed error information from logs or alerting solutions
- Reproduce and debug in a local installation. See [How to debug](../code/debugging.md) for more information.
- Checkout the [troubleshooting documentation](../../install/troubleshoot.md) and reach out on Discord.
